...
public final class MockHandler implements FieldHandler<Mock> {
    ...
    public void handle(final Mock annotation, final Field element) throws HandleException {
        ...
        if (annotation.providedBy().length() > 0 ) {
            ...
            try {
                ...
                if (!element.getType().isAssignableFrom(method.getReturnType())) {
                    throw new HandleException("Impossible to mock "
                            + element.getDeclaringClass().getName()
                            + " due to compatibility type, method provider "
                            + providedClass.getName() + "#"
                            + annotation.providedBy() + " returns "
                ...
                try {
                    ...
                    throw new HandleException("Impossible to mock "
                            ...
                            + ", method provider "
                            + providedClass.getName() + "#"
                            + annotation.providedBy() + " raised an error", t);
                }
            } catch (SecurityException e) {
                throw new HandleException("Impossible to mock "
                        + element.getDeclaringClass().getName()
                        + ", impossible to access to method provider "
                        + providedClass.getName() + "#"
                ...
                throw new HandleException("Impossible to mock "
                        ...
                        + ", the method provider "
                        + providedClass.getName() + "#"
                        + annotation.providedBy() + " doesn't exist.");
            }
        } else {
    ...
    private <T> T getMockProviderForType(T t, Method method, Class<?> cls)
        ...
        if (method.getReturnType() == t) {
            try {
                if (logger.isDebugEnabled()) {
                    logger.debug("        ...invoke Provider method for Mock: "
                            + method.getName());
                }
        ...
        throw new HandleException("The method: " + method
                + " shuld be return a type " + t);
    }
